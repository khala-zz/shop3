...
class ProductController extends Controller
{
	...
	public function __construct(Product $product,Category $category,CategoryFeature $categoryFeature, Setting $setting,ProductFeature $productFeature, Coupon $coupon, DeliveryAddress $delivery_address, Order $order, Review $review){
	...
	public function detail($slug, $id)
	{
    	...
    	//get setting
		$settings = $this -> setting -> select('setting_key','setting_value') -> where('is_active',1) ->whereNotIn('id',[4,7])->get();

    	//get setting chinh sách
		$setting_chinh_sach = $this -> setting -> select('setting_key','setting_value') -> where('is_active',1) -> where('id',1) -> first() ->dd();
		...
		$products_related = $this -> product  -> where([
		...
		return view('frontend.product.detail',compact('product','products_featured','settings','products_related','setting_chinh_sach','categoryFeatures', 'reviews'));
	}
    //change price
	...
	public function order(Request $request){
        ...
        return view('frontend.product.order',compact('userDetails','shipping_details','userCart','payment_value','shipping_value'));
	}
