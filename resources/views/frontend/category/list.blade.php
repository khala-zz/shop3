...
@extends('frontend.layouts.master')
	...
	<main class="main">
			<div class="page-header bg-dark"
				style="background-image: url('{{ asset('uploads/categories/'.$category -> image) }}'); background-color: #3C63A4;">
				<h3 class="page-subtitle">{{ $category -> title }}</h3>
							...
							<div class="sidebar-content">
								<div class="sticky-sidebar" data-sticky-options="{'top': 10}">
									<div class="filter-actions">
										...
										<a href="#" onclick="location.href='{{url()->current()}}';" class="filter-clean text-primary">Bỏ lọc</a>
									</div>
									<div class="widget widget-collapsible">
                                        ...
                                        <ul class="widget-body filter-items search-ul">
                                            ...
                                            @if( $item -> parent == null)
                                            <li>
                                                <a href="{{ route('category.product',['slug' => slugify($item -> title,'-'),'id' => $item -> id])}}" class="{{ count($item->children) ? 'show' :'' }} {{Request::segment(3) == $item-> id ? 'active-filters' : ''}}">{{$item->title}}</a>
									...
									<div class="widget widget-collapsible">
										...
										<ul class="widget-body filter-items">
											@foreach($category_feature_sizes  as $category_feature )
											<li><a href="#">{{$category_feature ->  field_title }}</a></li>
											@endforeach
										</ul>
									</div>
									<div class="widget widget-collapsible">
										...
										<ul class="widget-body filter-items">
											@foreach($category_feature_colors  as $category_feature )
											<li><a href="#">{{$category_feature ->  field_title }}</a></li>
											@endforeach
										</ul>
									</div>
									...
									<div class="widget widget-collapsible">
										...
										<ul class="widget-body filter-items-khala">
											...
											<li><a href="{{url()->current().'?brand_filter='.$brand -> id}}" class="{{Request::get('brand_filter') == $brand -> id ? 'active-filters' :''}}">{{ $brand -> title}}<span>{{ $brand -> products_count}}</span></a></li>
											@endforeach
...
'margin': 20											}">
						...
						<div class="col-lg-9 main-content">
							...
							<div class="row cols-2 cols-sm-3 product-wrapper">
								...
								@empty
									<div class="product-wrap">
										Không có sản phẩm phù hợp với thông tin bạn lọc vui lòng <a href="{{url()->current()}}"><strong>tìm lại</strong> </a>, cám ơn !
									</div>
								@endforelse
